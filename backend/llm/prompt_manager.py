...
class PromptManager:
    """
    Central repository for all system and user prompt templates.
    Isolates the string structures completely away from specific business agents.
    """
...
"""
    ...
    def get_prompt(cls, key: str, **kwargs: Any) -> str:
        """
        Retrieves formatted prompt based on dictionary kwargs injection
        """
        template = getattr(cls, key.upper(), None)
        if not template:
